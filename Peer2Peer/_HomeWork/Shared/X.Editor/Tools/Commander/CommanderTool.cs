...
namespace X.Editor.Tools.Commander
{
    class CommanderTool : ToolWindowBase
    {
        ...
        void Hierarchy_SelectedItemChanged(object sender, System.Helpers.ItemEventArgs<HierarchyNode> e)
        {
            InternalList.Clear();

            foreach (var it in Shell.Commands) InternalList.Add(it);
            foreach (var it in e.Item.Commands) InternalList.Add(it);

            ApplyFilter();

            e.Item.Commands.ItemAdded += (ias, iaa) => InternalList.Add(iaa.Item);
            e.Item.Commands.ItemRemoved += (ias, iaa) => InternalList.Remove(iaa.Item);
        }
