...
namespace X.Editor
{
    public partial class Main : System.Windows.Forms.Form, IEditorShell
    {
        ...
        public Main(params string[] args)
        {
            ...
            {
                var window = DockPanel.GetOpenDocumentWindows().Where(x => x.Node.Equals(ara.Item)).FirstOrDefault();
                if (window != null) window.Activate();
                else
                {
                    var newWindow = new DocumentWindowBase(this, ara.Item);
                    newWindow.Show(this.DockPanel, DockState.Document);
                    //foreach (var doc in ara.Item.Documents)
                    //{
                    //    var newWindow = new DocumentWindowBase(this, ara.Item, doc);
                    //    newWindow.Show(this.DockPanel, DockState.Document);
                    //}
                }
            };
