...
namespace X.DataModel
{
    ...
    public class XObject : DynamicObject, INotifyPropertyChanged, IHaveXRules, IAutoInstanciate
    {
        ...
        protected virtual internal object InternalGet(string propName, bool avoidMissingHandler = false)
        {
            ...
            var prop = DynamicsCache
                .GetPropertiesAccessors(this.GetType())
                .FirstOrDefault(axx => axx.PropertyName == propName);

            if (prop != null) return prop.Property.PropertyType.Hype().GetDefaultValue();
        ...
        public T Get<T, U>(Expression<Func<T>> expression, Func<U, T> converter)
        {
            return converter((U)InternalGet(expression.ToPropertyName<T>()));
        }
        public void Set<T>(Expression<Func<T>> expression, T value)
        {
            InternalSet(expression.ToPropertyName<T>(), value);
        }
        public void Set<T, U>(Expression<Func<T>> expression, T value, Func<T, U> converter)
        {
            InternalSet(expression.ToPropertyName<T>(), converter(value));
        }
