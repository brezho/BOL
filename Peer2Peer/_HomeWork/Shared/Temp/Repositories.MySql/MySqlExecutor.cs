...
{
    class MySqlExecutor : AdoNetQueryExecutorBase
    {
        public MySqlExecutor(IDatabase database)
            : base(database)
        { }

        WhereClauseVisitor<T> GetVisitor<T>()
        {
            var res = new WhereClauseVisitor<T>();
            res.FieldClose = "";
            res.FieldOpen = "";
            return res;
        }
        ...
        public override void ExecuteSave<T>(string databaseObjectName, Analysis.SaveCommand<T> command)
        {
            ...
            var insertStatement = string.Format("insert ignore into {0}({1}) values({2});", databaseObjectName, insertColumns, values);
